<?php
    ...
    */
    ...
    if(isset($_GET['deleteEvent'])){
        $db->adminDelete("event", $_GET['id'], "idevent");
        $db->adminDelete("session", $_GET['id'], "event");
    }
    // Delete a session function.
    if(isset($_GET['deleteSession'])){
        $db->adminDelete("session", $_GET['id'], "idsession");
    }

    // Attending event function.
    if(isset($_GET['attendEvent'])){
        $db->attendEvent($_GET['id'], $_SESSION['username']);
    }
    // Attending session function.
    if(isset($_GET['attendSession'])){
        $db->attendSession($_GET['id'], $_SESSION['username']);
    }
        ...
        <div class="section container s11 m11">
            <?php
                ...
                if(($_SESSION['userType'] == "manager") || ($_SESSION['userType'] == "admin")){
                    ...
                    foreach($result as $response){
                        echo '<h6 class="center">Event ' . $response["idevent"] . '</h6><form action="" method="post">
                            ...
                            Venue:<input readonly type="text" name="eventVenue" value="' . ($db->getVenue($response["venue"]))["name"] . '"><br/>';
                        ...
                        echo '<a class="btn red darken-2" href="events.php?deleteEvent=true&id=' . $response["idevent"] . '">Delete 
                            Event</a>';
                        echo '</form><br/>';
                        $result2 = $db->getSessionByEvent($response["idevent"]);
                        if(!($result2 == null)){
                ...
                foreach($result as $response){
                    ...
                    $attending = false;
                    foreach($db->getRegisteredEvents($_SESSION['username']) as $res) 
                ...
                foreach($result as $response){
                    echo '<form action="" method="post">
                        ...
                        Event:<input readonly type="text" name="sessEvent" value="' . ($db->getEvent($response["event"]))["name"] . '"><br/>';
                    // Check if they are attending this session already.
                    $attending2 = false;
                    foreach($db->getRegisteredSessions($_SESSION['username']) as $res) 
