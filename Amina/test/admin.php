...
<body>
  ...
  <div class="container">
    ...
    <div class="table-responsive">
      <h3 align="center">User table</h3><br>
    ...
    <hr>
    <div class="table-responsive">
      <h3 align="center">Sport table</h3><br>
      <div id="sport_table"></div>
    </div><br>
    <br>
    <br>
    <hr>
...
<script>
  ...
  $(document).ready(function(){//Someone touchs on any action events  
       function fetch_data() // Get info from php files  
       {  
            $.ajax({  
                 url:"AjaxFolders/userAjax/select.php",  
                 method:"POST",  
       ...
       fetch_data();  //Display the table
       $(document).on('click', '#btn_add', function(){  //Someone types what they want to do with most validation  
            var username = $('#username').text();  
            var role = parseInt($('#role').text()); 
            var password =  $('#password').text();  
            var team = parseInt($('#team').text()); 
            var league = parseInt($('#league').text());
            if(username == '')  
            {  
            ...
            $.ajax({  //the user insert new values 
                 url:"AjaxFolders/userAjax/insert.php",  
                 method:"POST",  
       ...
       function edit_data(username, oldusername, role, password, team, league, getdata)//Clicking on the work or number to edit it   
       {  
           console.log('sent edit data');
            $.ajax({  
                 url:"AjaxFolders/userAjax/edit.php",  
                 method:"POST",  
                 data:{username:username, role:role, password:password, team:team, league:league, getdata:getdata, oldusername:oldusername },  
                 dataType:"text",  
                 success:function(data){  
       ...
       $(document).on('blur', '.username', function(){ //Directly edit on one thing at the one time. Also, we need to control the updates by restricting another id to effect tables  
            //var usernamedata = $(this).data("username");  
            var parent = $(this).parent();
            var username = parent.find('.username').text();  
            var oldusername = parent.find('.username').attr('data-old');
            var role = parseInt(parent.find('.role').text()); 
            var password =  parent.find('.password').text(); 
            var team = parseInt(parent.find('.team').text());
            var league = parseInt(parent.find('.league').text());
            console.log('[username='+username+',olduser='+oldusername+',role='+role+',password='+password+',team='+team+',league='+league+']');
            edit_data(username, oldusername ,role, password, team, league, "username");  
        ...
        $(document).on('blur', '.password', function(){  
            ...
            var league = parseInt(parent.find('.league').text());
            console.log('[username='+username+',olduser='+oldusername+',role='+role+',password='+password+',team='+team+',league='+league+']');
            edit_data(username, oldusername ,role, password, team, league, "password");   
       });
       $(document).on('blur', '.role', function(){  
            ...
            var league = parseInt(parent.find('.league').text());
            console.log('[username='+username+',olduser='+oldusername+',role='+role+',password='+password+',team='+team+',league='+league+']');
            edit_data(username, oldusername ,role, password, team, league, "role");     
       });  
       $(document).on('blur', '.team', function(){  
            ...
            var league = parseInt(parent.find('.league').text());
            console.log('[username='+username+',olduser='+oldusername+',role='+role+',password='+password+',team='+team+',league='+league+']');
            edit_data(username, oldusername ,role, password, team, league, "team");    
       });
       $(document).on('blur', '.league', function(){  
             //var usernamedata = $(this).data("username");  
            var parent = $(this).parent();
            var username = parent.find('.username').text();  
            var oldusername = parent.find('.username').attr('data-old');
            ...
            var league = parseInt(parent.find('.league').text());
            console.log('[username='+username+',olduser='+oldusername+',role='+role+',password='+password+',team='+team+',league='+league+']');
            edit_data(username, oldusername ,role, password, team, league, "league");   
       });


       $(document).on('click', '.btn_delete', function(){  //Delete one row
              var parent = $(this).parent().parent();
              var username = parent.find('.username').text();   
            if(confirm("Are you sure you want to delete this?"))  
            {  
                 $.ajax({  
                      url:"AjaxFolders/userAjax/delete.php",  
                      method:"POST",  
  ...
  //As you can see code below here. They are mostly similar as above here, but they have different number of parameters.
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////Sport
         function sportfetch_data() {
            $.ajax({  
                 url:"AjaxFolders/sportAjax/select.php",  
                 method:"POST",  
                 success:function(data){  
       ...
       sportfetch_data();
       $(document).on('click', '#sport_btn_add', function(){  
            var name = $('#name').text();  
            ...
            $.ajax({  
                 url:"AjaxFolders/sportAjax/insert.php",  
                 method:"POST",  
                 data:{name:name},  
                 dataType:"text",  
                 success:function(data)  
       ...
       function sport_edit_data(id,name,getdata)  
       {  
            $.ajax({  
                 url:"AjaxFolders/sportAjax/edit.php",  
                 method:"POST",  
                 data:{id:id,name:name,getdata:getdata},  
                 dataType:"text",  
                 success:function(data){  
       ...
       $(document).on('blur', '.name', function(){  
            ...
            sport_edit_data(id,name, "name");  
            //parent.find('.username').attr('data-old', username);
       });  


       $(document).on('click', '.sport_btn_delete', function(){  
              var parent = $(this).parent().parent();
            ...
            {  
                 $.ajax({  
                      url:"AjaxFolders/sportAjax/delete.php",  
                      method:"POST",  
                      data:{id:id},  
                      dataType:"text",  
                      success:function(data){  
